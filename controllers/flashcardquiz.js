...
const create = async (req, res) => {
  ...
  try {
    ...
    // Save to database
    const flashcardquiz = await FlashcardQuiz.create(newDoc);
    console.log('Created Document:', created); // Debug log
    res.status(201).json({ msg: 'FlashcardQuiz created successfully.', id: created._id });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Server error. Could not create flashcardquiz.' });
  }
};
...
const read = async (req, res) => {
  ...
  try {
    ...
    const flashcardquiz = await FlashcardQuiz.find(query).select(field.join(' '));
    console.log('Found Data:', data); // Debug log
    const stack = flashcardquiz.map(doc => {
      let obj = {};
      field.forEach(f => {
        obj[f] = doc[f];
      });
...
const deletE = async (req, res) => {
  ...
  try {
    ...
    await FlashcardQuiz.deleteOne(query);
